 ...
 ******************************************************************************/
package com.adobe.aem.communities.loader;
...
import com.adobe.aem.communities.loader.Hostname;
...
public class CreateCommunities extends org.apache.sling.api.servlets.SlingAllMethodsServlet {
	...
	protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response) throws ServerException, IOException {
		...
		String nomultilingualParam = (String) request.getParameter("nomultilingual");
		String compositeParam = (String) request.getParameter("composite");
		...
		boolean nomultilingual = (nomultilingualParam!=null && nomultilingualParam.length()>0)?true:false;
		boolean composite = (compositeParam!=null && compositeParam.length()>0)?true:false;
				...
				for (Resource resConfigFile: resConfigFiles.getChildren()) {
					if (resConfigFile!=null && resConfigFile.getName().startsWith(configOption.replace("setup-", "")) && (resConfigFile.getName().toLowerCase().endsWith(".csv"))) {
						...
						if (resConfigFile.getName().contains("author")) {
							Loader.processLoading(resourceResolver, in, hostname_author, port_author, port, password, analytics, false, true, minimize, enablement, nomultilingual, composite, csvPath, maxretries);
						} else {
							Loader.processLoading(resourceResolver, in, hostname, port, port, password, analytics, false, true, minimize, enablement, nomultilingual, composite, csvPath, maxretries);
						}
